...
namespace ModCompilKenobi
{
  public static class KeyboardConfigs {
    ...
    static KeyboardConfigs() {
      ...
      Configs[j].Left = new Keys[] { Keys.Divide };
      Configs[j].Right = new Keys[] { Keys.Multiply };
      Configs[j].Jump = new Keys[] { Keys.NumPad7 };
      Configs[j].Shoot = new Keys[] { Keys.F7 };
      Configs[j].Dodge = new Keys[] { Keys.Subtract };

      j++;
      Configs[j].Down = new Keys[] { Keys.F14 };
      Configs[j].Up = new Keys[] { Keys.F15 };
      Configs[j].Left = new Keys[] { Keys.F16 };
      Configs[j].Right = new Keys[] { Keys.F17 };
