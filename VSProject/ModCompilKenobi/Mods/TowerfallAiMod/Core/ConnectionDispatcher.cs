...
namespace TowerfallAi.Core {
  public class ConnectionDispatcher {
    ...
    /**
    * Receive message from client  (mode level fps agentTimeout agents[])
    */
    private void HandleConfigMessage(RemoteConnection connection, Message message) {
      try
      {
        AiMod.ValidateConfig(message.config);
        int nbRemoteConnections = Agents.CountRemoteConnections(message.config.agents);

        int nbHuman = Agents.CountHumanConnections(message.config.agents);
        int nbSlotOpen = Agents.getNumberOfSlotOpen(nbHuman);

        if (nbRemoteConnections > nbSlotOpen)
        {
          nbRemoteConnections = nbSlotOpen;
          // if human keep it
          // keep only nbRemoteConnections
          List<AgentConfig> tmp = new List<AgentConfig>();
          int nbRemote = 0;
          foreach (var item in message.config.agents)
          {
            if (nbRemote == nbSlotOpen)
            {
              message.config.agents = tmp;
              Logger.Info(message.ToString());

              break;
            }
            if (item.type == AgentConfig.Type.Human)
            {
              tmp.Add(item);
              continue;
            }
            if (item.type == AgentConfig.Type.Remote)
            {
              tmp.Add(item);
              nbRemote++;
            }
        ...
        int connectionsRequired = nbRemoteConnections;
        lock (joinLock)
        {
