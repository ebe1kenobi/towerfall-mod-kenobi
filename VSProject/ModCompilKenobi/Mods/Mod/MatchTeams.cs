...
namespace TowerfallAi.Mod
{
    ...
    public class MyMatchTeams : MatchTeams
    {
        ...
        public override int GetAmountOfPlayersOfAllegiance(Allegiance allegiance)
        {
            if (!TF8PlayerMod.TF8PlayerMod.Mod8PEnabled)
            {
              return base.GetAmountOfPlayersOfAllegiance(allegiance);
            }
            int num = 0;
        ...
        public override List<int> GetPlayersOfAllegiance(Allegiance allegiance)
        {
            if (!TF8PlayerMod.TF8PlayerMod.Mod8PEnabled)
            {
              return base.GetPlayersOfAllegiance(allegiance);
            }
        ...
        public override bool ProperlyAssigned
        {
            get
            {
                bool flag = false;
                bool flag2 = false;
                for (int i = 0; i < (TF8PlayerMod.TF8PlayerMod.Mod8PEnabled ? 8 : 4); i++)
                {
                    if (TFGame.Players[i])
                    {
